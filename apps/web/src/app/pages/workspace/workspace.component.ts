...
@Component({standalone:true,imports:[RouterLink,ReviewRowComponent,StatusPillComponent,SurfaceCardComponent,EmptyStateComponent],template:`
...
<div class="grid"><ui-surface-card eyebrow="Workspace history" title="Review runs"><a actions routerLink="/reviews">View all →</a>@if(store.loading()){<div class="list-state">Loading review history…</div>}@else if(store.error()){<div class="list-state error">{{store.error()}}</div>}@else if(store.reviews().length===0){<ui-empty-state title="No review runs yet" message="Open or update a pull request in an installed repository. Its review will appear here automatically."/>}@else{@for(review of store.reviews().slice(0,6);track review.id;let first=$first){<review-row [review]="review" [latest]="first"/>}}</ui-surface-card>
